...
void level2(mainScreen &game){//blocks moving down
    ...
    for(int j = startCol; j <= game.col + startCol; j++)
        for(int i = min(game.row + endRow - 1, 8); i >= endRow; i--){
            if(getData(game.pHead[i], j) == -1){
                for(int key = i; key >= endRow; key--){
                    if(getData(game.pHead[key], j) != -1 && getData(game.pHead[key - 1], j) == -1)
                        changePosData(game.pHead[key - 1], j, -1);
                    changePosData(game.pHead[key], j, getData(game.pHead[key - 1], j));
                }
            }
            if(getData(game.pHead[i], j) == -1){
                for(int key = i; key >= endRow; key--){
        ...
        for(int j = startCol; j <= game.col + startCol - 1; j++){
            if(getData(game.pHead[i], j) == -1){
                for(int key = j; key <= game.col + startCol - 1; key++){
                    if(getData(game.pHead[i], key) != -1 && getData(game.pHead[i], key + 1) == -1)
                        changePosData(game.pHead[i], key + 1, -1);
                    changePosData(game.pHead[i], key, getData(game.pHead[i], key + 1));
                }
            }
            if(getData(game.pHead[i], j) == -1){
                for(int key = j; key <= game.col + startCol - 1; key++){
...
void drawUnKey(mainScreen &game, int yr, int xr, char art[50][120], int y1, int x1){
    if(getData(game.pHead[yr], xr) == -1){
        ...
        if(yr == y1 && xr == x1)
            ...
            setColor(getData(game.pHead[yr], xr)%6 + 9);
        
        char show;
        if(getData(game.pHead[yr], xr) >= 0 && getData(game.pHead[yr], xr) < 26)
            show = (char)(getData(game.pHead[yr], xr) + (int)'A');
        else if(getData(game.pHead[yr], xr) >= 26)
            show = (char)(getData(game.pHead[yr], xr) + (int)'a' - 26);
...
void drawKey(mainScreen &game, int yr, int xr, int y1, int x1){
    if(yr == y1 && xr == x1)
        ...
        setColor(16 + getData(game.pHead[yr], xr)%6 + 9);
...
void drawHint(mainScreen &game, int y, int x){
    ...
    char show;
    if(getData(game.pHead[y], x) >= 0 && getData(game.pHead[y], x) < 26)
        show = (char)(getData(game.pHead[y], x) + (int)'A');
    else if(getData(game.pHead[y], x) >= 26)
        show = (char)(getData(game.pHead[y], x) + (int)'a' - 26);
...
void drawingLine(mainScreen &game, int i, int j){
    ...
    //line down
    if(getData(game.pHead[i/3], j) == (int)'0'){
        if(i%3 == 0)
            cout << (char)(201) << (char)(205) << (char)(205) << (char)(205) << (char)(187);
        else if(i%3 == 1)
    ...
    //line up
    else if(getData(game.pHead[i/3], j) == (int)'1'){
        if(i%3 == 0)
    ...
    //line right
    else if(getData(game.pHead[i/3], j) == (int)'2'){
        if(i%3 == 0)
            cout << (char)(201) << (char)(205) << (char)(205) << (char)(205) << (char)(187);
        else if(i%3 == 1)
    ...
    //line left
    else if(getData(game.pHead[i/3], j) == (int)'3'){
        if(i%3 == 0)
            cout << (char)(201) << (char)(205) << (char)(205) << (char)(205) << (char)(187);
        else if(i%3 == 1)
    ...
    //line |
    else if(getData(game.pHead[i/3], j) == (int)'4'){
        gotoxy(j*5 + 3, i + 2);
        cout                                   << (char)(179);
    }
    //line -
    else if(getData(game.pHead[i/3], j) == (int)'5'){
        gotoxy(j*5 + 1, i + 2);
        if(i%3 == 1)
            cout << (char)(196) << (char)(196) << (char)(196) << (char)(196) << (char)(196);
    }
    //line down-right
    else if(getData(game.pHead[i/3], j) == (int)'6'){
        gotoxy(j*5 + 3, i + 2);
        if(i%3 == 1)
    ...
    //line down-left
    else if(getData(game.pHead[i/3], j) == (int)'7'){
        if(i%3 == 1){
            gotoxy(j*5 + 1, i + 2);
    ...
    //line up-right
    else if(getData(game.pHead[i/3], j) == (int)'8'){
        gotoxy(j*5 + 3, i + 2);
        if(i%3 == 0)
            cout                               << (char)(179);
        else if(i%3 == 1)
    ...
    //9: 217: line up-left
    else if(getData(game.pHead[i/3], j) == (int)'9'){
        if(i%3 == 0){
            gotoxy(j*5 + 3, i + 2);
            cout                               << (char)(179);
        }
	...
	for(int i = 0; i < (10)*3; i++){
		for(int j = 0; j < 22; j++){
            if(i < (game.row + 2)*3 && j < game.col + 2){
                if(getData(game.pHead[i/3], j) >= (int)'0' && getData(game.pHead[i/3], j) <= (int)'9'){
                    drawingLine(game, i, j);
                }
                else{
                    //showing color from 8 to 14
                    setColor(getData(game.pHead[i/3], j)%6 + 9);
                    gotoxy(j*5 + 1, i + 2);
                    if(getData(game.pHead[i/3], j) == -1){
                        //15 = 0*16 + 15 white text black background
                        setColor(15);
                        ...
                        cout << (char)(201) << (char)(205) << (char)(205)                                         << (char)(205) << (char)(187);
                    else if(i%3 == 1 && getData(game.pHead[i/3], j) < 26)
                        cout << (char)(186) << " "         << (char)(getData(game.pHead[i/3], j) + (int)'A')      << " "         << (char)(186);
                    else if(i%3 == 1 && getData(game.pHead[i/3], j) >= 26)
                        cout << (char)(186) << " "         << (char)(getData(game.pHead[i/3], j) + (int)'a' - 26) << " "         << (char)(186);
                    else if(i%3 == 2)
                        cout << (char)(200) << (char)(205) << (char)(205)                                         << (char)(205) << (char)(188);
                }
            }
...
void drawOutsideBoard(int &level){
    if(level == 1){
        //1 draw blue border
        drawBorder(1);
        //40 = 1*16 + 15 white text blue background
